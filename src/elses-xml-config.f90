...
!      related file : elses-qm-engine.f90
    ...
    ! get <external_force> node
    calc%ext_force%set  = .false.
    calc%ext_force%mode = 'not_set'
    calc%ext_force%field_amplitude = 0.0d0
    work_node1 => getFirstElementByTagName(calc_node,"external_force")
    if ( associated(work_node1) ) then
      calc%ext_force%set  = .true.
      if (log_unit > 0) write(log_unit,'(a)') &
         & 'INFO-XML:Optional tag: external_force'
      work_node2 => getFirstElementByTagName(work_node1,"field_amplitude")
      if( associated(work_node2) ) then
       value = getChildValue(work_node2)
       read(unit=value,fmt=*,iostat=ierr) calc%ext_force%field_amplitude
       if (ierr /= 0) then
         write(*,*)'ERROR in XML reading (external force:field amplitude)'
         stop
       endif
       unit  = getAttribute(work_node2,"unit")
       if ( unit == "" ) unit = "a.u."
       calc%ext_force%field_amplitude=calc%ext_force%field_amplitude * XML_TO_AU(unit)
       if (log_unit > 0) write(log_unit,*) &
         & 'INFO-XML:Optional tag: external_force: field amplitude=', calc%ext_force%field_amplitude
      endif
    endif
!
    ! get <wavekernel> node
    work_node1 => getFirstElementByTagName(calc_node,"wavekernel")
    if ( .not. associated(work_node1) ) then
      work_node1 => getFirstElementByTagName(calc_node,"wave_packet")  ! old tag name
    end if
    calc%wavekernel%set = .false. ! default setting
    calc%wavekernel%mode = "off"  ! default setting
    if ( associated(work_node1) ) then
      calc%wavekernel%set =.true.
      if (getAttribute(work_node1,"mode") == "on") calc%wavekernel%mode ="on"

      calc%wavekernel%limit_t = 0d0  ! dummy value
      calc%wavekernel%replace_t = 0d0  ! dummy value
      if (calc%wavekernel%mode == "on") then
        ! get nodes for time config
        ! get <delta_t> node (required)
        work_node2 => getFirstElementByTagName(work_node1,"delta_t")
        if( associated(work_node2) ) then
          value = getChildValue(work_node2)
          read(unit=value,fmt=*) calc%wavekernel%delta_t
          unit  = getAttribute(work_node2,"unit")
          if ( unit == "" ) then
            unit = "a.u."
          end if
          calc%wavekernel%delta_t = calc%wavekernel%delta_t * XML_TO_AU(unit)
        else
          call XML_error("<delta_t> not found for wavekernel node")
        end if
        ! get <limit_t> node
        work_node2 => getFirstElementByTagName(work_node1,"limit_t")
        if( associated(work_node2) ) then
          value = getChildValue(work_node2)
          read(unit=value,fmt=*) calc%wavekernel%limit_t
          unit  = getAttribute(work_node2,"unit")
          if ( unit == "" ) then
            unit = "a.u."
          end if
          calc%wavekernel%limit_t = calc%wavekernel%limit_t * XML_TO_AU(unit)
        end if
        ! get <replace_t> node
        work_node2 => getFirstElementByTagName(work_node1,"replace_t")
        if( associated(work_node2) ) then
          value = getChildValue(work_node2)
          read(unit=value,fmt=*) calc%wavekernel%replace_t
          unit  = getAttribute(work_node2,"unit")
          if ( unit == "" ) then
            unit = "a.u."
          end if
          calc%wavekernel%replace_t = calc%wavekernel%replace_t * XML_TO_AU(unit)
        end if
      end if

      ! get nodes for perturbation (H1) config
      calc%wavekernel%h1_type = ""  ! default setting
      calc%wavekernel%charge_factor_common = -1d0  ! dummy value
      calc%wavekernel%charge_factor_H = -1d0  ! dummy value
      calc%wavekernel%charge_factor_C = -1d0  ! dummy value
      calc%wavekernel%temperature = -1d0  ! dummy value
      calc%wavekernel%perturb_interval = -1d0  ! dummy value
      calc%wavekernel%eigenstate_damp_constant = -1d0  ! dummy value
      work_node2 => getFirstElementByTagName(work_node1,"perturbation")
      if ( associated(work_node2) ) then
        calc%wavekernel%h1_type = getAttribute(work_node2,"type")
        ...
        if( associated(work_node3) ) then
          ...
          read(unit=value,fmt=*) calc%wavekernel%charge_factor_common
        end if
        ! get <charge_factor_H> node
        work_node3 => getFirstElementByTagName(work_node2,"charge_factor_H")
        if( associated(work_node3) ) then
          value = getChildValue(work_node3)
          read(unit=value,fmt=*) calc%wavekernel%charge_factor_H
        end if
        ! get <charge_factor_C> node
        work_node3 => getFirstElementByTagName(work_node2,"charge_factor_C")
        if( associated(work_node3) ) then
          value = getChildValue(work_node3)
          read(unit=value,fmt=*) calc%wavekernel%charge_factor_C
        end if
        ! get <temperature> node
        work_node3 => getFirstElementByTagName(work_node2,"temperature")
        if( associated(work_node3) ) then
          value = getChildValue(work_node3)
          read(unit=value,fmt=*) calc%wavekernel%temperature
        end if
        ! get <perturb_interval> node
        work_node3 => getFirstElementByTagName(work_node2,"perturb_interval")
        if( associated(work_node3) ) then
          value = getChildValue(work_node3)
          read(unit=value,fmt=*) calc%wavekernel%perturb_interval
        end if
        ! get <eigenstate_damp_constant> node
        work_node3 => getFirstElementByTagName(work_node2,"eigenstate_damp_constant")
        if( associated(work_node3) ) then
          value = getChildValue(work_node3)
          read(unit=value,fmt=*) calc%wavekernel%eigenstate_damp_constant
        end if
      end if

      ! get nodes for filter config
      calc%wavekernel%filter_mode = ""  ! default setting
      calc%wavekernel%fst_filter = -1  ! dummy value
      calc%wavekernel%end_filter = -1  ! dummy value
      calc%wavekernel%filter_group_filename = ""  ! dummy value
      work_node2 => getFirstElementByTagName(work_node1,"filter")
      if ( associated(work_node2) ) then
        calc%wavekernel%filter_mode = getAttribute(work_node2,"type")
        ! get <fst_filter> node
        work_node3 => getFirstElementByTagName(work_node2,"fst_filter")
        if( associated(work_node3) ) then
          value = getChildValue(work_node3)
          read(unit=value,fmt=*) calc%wavekernel%fst_filter
        end if
        ! get <end_filter> node
        work_node3 => getFirstElementByTagName(work_node2,"end_filter")
        if( associated(work_node3) ) then
          value = getChildValue(work_node3)
          read(unit=value,fmt=*) calc%wavekernel%end_filter
        end if
        ! get <filter_group_filename> node
        work_node3 => getFirstElementByTagName(work_node2,"filter_group_filename")
        if( associated(work_node3) ) then
          calc%wavekernel%filter_group_filename = trim(getChildValue(work_node3))
        end if
      end if

      ! get nodes for initializatin config
      calc%wavekernel%init_type = ""  ! default setting
      calc%wavekernel%alpha_delta_multiple_indices_str = ""  ! default setting
      calc%wavekernel%alpha_delta_index = -1  ! dummy value
      calc%wavekernel%phase_factor_coef = 0d0  ! default setting
      calc%wavekernel%to_multiply_phase_factor = .false.  ! default setting
      work_node2 => getFirstElementByTagName(work_node1,"initialization")
      if ( associated(work_node2) ) then
        calc%wavekernel%init_type = getAttribute(work_node2,"type")
        ! get <alpha_delta_index> node
        work_node3 => getFirstElementByTagName(work_node2,"alpha_delta_index")
        if( associated(work_node3) ) then
          value = getChildValue(work_node3)
          read(unit=value,fmt=*) calc%wavekernel%alpha_delta_index
        end if
        ! get <phase_factor_coef> node (optional)
        work_node3 => getFirstElementByTagName(work_node2,"phase_factor_coef")
        if( associated(work_node3) ) then
          value = getChildValue(work_node3)
          read(unit=value,fmt=*) calc%wavekernel%phase_factor_coef
          calc%wavekernel%to_multiply_phase_factor = .true.
        end if
        ! get <localize_potential_depth> node 
        work_node3 => getFirstElementByTagName(work_node2,"localize_potential_depth")
        if( associated(work_node3) ) then
          value = getChildValue(work_node3)
          read(unit=value,fmt=*) calc%wavekernel%localize_potential_depth
        end if
        ! get <alpha_delta_multiple_indices> node
        work_node3 => getFirstElementByTagName(work_node2,"alpha_delta_multiple_indices")
        if( associated(work_node3) ) then
          calc%wavekernel%alpha_delta_multiple_indices_str = getChildValue(work_node3)
        end if
        ! get <alpha_delta_min_x> node
        work_node2 => getFirstElementByTagName(work_node1,"alpha_delta_min_x")
        if( associated(work_node2) ) then
          value = getChildValue(work_node2)
          read(unit=value,fmt=*) calc%wavekernel%alpha_delta_min_x
        end if
        ! get <alpha_delta_max_x> node
        work_node2 => getFirstElementByTagName(work_node1,"alpha_delta_max_x")
        if( associated(work_node2) ) then
          value = getChildValue(work_node2)
          read(unit=value,fmt=*) calc%wavekernel%alpha_delta_max_x
        end if
        ! get <localize_start> node
        work_node2 => getFirstElementByTagName(work_node1,"localize_start")
        if( associated(work_node2) ) then
          value = getChildValue(work_node2)
          read(unit=value,fmt=*) calc%wavekernel%localize_start
        end if
        ! get <localize_end> node
        work_node2 => getFirstElementByTagName(work_node1,"localize_end")
        if( associated(work_node2) ) then
          value = getChildValue(work_node2)
          read(unit=value,fmt=*) calc%wavekernel%localize_end
        end if
      end if

      ! get nodes for re-initialization after matrix replacement config
      calc%wavekernel%re_initialize_method = ""  ! default setting
      work_node2 => getFirstElementByTagName(work_node1,"re_initialize")
      if ( associated(work_node2) ) then
        calc%wavekernel%re_initialize_method = getAttribute(work_node2,"type")
        if ( trim(calc%wavekernel%re_initialize_method) == "minimize_lcao_error_cutoff" ) then
          work_node3 => getFirstElementByTagName(work_node2,"cutoff_residual")
          if( associated(work_node3) ) then
            value = getChildValue(work_node3)
            read(unit=value,fmt=*) calc%wavekernel%vector_cutoff_residual
          end if
        else if ( trim(calc%wavekernel%re_initialize_method) == "minimize_lcao_error_suppress" .or. &
             trim(calc%wavekernel%re_initialize_method) == "minimize_lcao_error_matrix_suppress" .or. &
             trim(calc%wavekernel%re_initialize_method) == "minimize_lcao_error_matrix_suppress_orthogonal" .or. &
             trim(calc%wavekernel%re_initialize_method) == "minimize_lcao_error_matrix_suppress_adaptive" .or. &
             trim(calc%wavekernel%re_initialize_method) == "minimize_lcao_error_matrix_suppress_select" ) then
          work_node3 => getFirstElementByTagName(work_node2,"suppress_const")
          if( associated(work_node3) ) then
            value = getChildValue(work_node3)
            read(unit=value,fmt=*) calc%wavekernel%suppress_constant
          end if
        end if
      end if

      ! get nodes for time evolution config
      calc%wavekernel%time_evolution_mode = ""  ! default setting
      work_node2 => getFirstElementByTagName(work_node1,"time_evolution")
      if ( associated(work_node2) ) then
        calc%wavekernel%time_evolution_mode = getAttribute(work_node2,"type")
      end if

      ! get nodes for message config
      calc%wavekernel%amplitude_print_threshold = -1d0  ! dummy value
      calc%wavekernel%amplitude_print_interval = -1d0  ! dummy value
      ...
      if ( associated(work_node2) ) then
        ...
        if( associated(work_node3) ) then
          value = getChildValue(work_node3)
          read(unit=value,fmt=*) calc%wavekernel%amplitude_print_threshold
        end if
        ! get <amplitude_print_interval> node
        work_node3 => getFirstElementByTagName(work_node2,"amplitude_print_interval")
        if( associated(work_node3) ) then
          value = getChildValue(work_node3)
          read(unit=value,fmt=*) calc%wavekernel%amplitude_print_interval
        end if
      end if

      ! get nodes for output config
      calc%wavekernel%output_filename = ""  ! default setting
      calc%wavekernel%is_binary_output_mode = .false.  ! default setting
      calc%wavekernel%is_output_split = .false.  ! default setting
      calc%wavekernel%output_interval = -1  ! dummy value
      calc%wavekernel%num_steps_per_output_split = -1  ! dummy value
      work_node2 => getFirstElementByTagName(work_node1,"wavekernel_output")
      if ( associated(work_node2) ) then
        calc%wavekernel%is_binary_output_mode = (getAttribute(work_node2,"format") == "binary")
        calc%wavekernel%is_output_split = (getAttribute(work_node2,"split") == "on")
        ! get <filename> node
        work_node3 => getFirstElementByTagName(work_node2,"filename")
        if( associated(work_node3) ) then
          calc%wavekernel%output_filename = trim(getChildValue(work_node3))
        end if        
        ! get <output_interval> node
        work_node3 => getFirstElementByTagName(work_node2,"output_interval")
        if( associated(work_node3) ) then
          value = getChildValue(work_node3)
          read(unit=value,fmt=*) calc%wavekernel%output_interval
        end if
        ! get <num_steps_per_output_split> node
        work_node3 => getFirstElementByTagName(work_node2,"num_steps_per_output_split")
        if( associated(work_node3) ) then
          value = getChildValue(work_node3)
          read(unit=value,fmt=*) calc%wavekernel%num_steps_per_output_split
        end if
      end if

      ! get nodes for overlap matrix config
      calc%wavekernel%is_overlap_ignored = .false.  ! default setting
      work_node2 => getFirstElementByTagName(work_node1,"overlap")
      if ( associated(work_node2) ) then
        calc%wavekernel%is_overlap_ignored = (getAttribute(work_node2,"ignored") == "on")
      end if
    endif  ! end of <wavekernel> node
    if ( calc%wavekernel%set ) then
      if (log_unit > 0) write(log_unit,'(a,a)') &
           & 'INFO-XML:Optional tag: wavekernel; mode = ',trim(calc%wavekernel%mode)
    endif
    return
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine output_load( output, output_node )
    ...
    if ( associated(node) ) then
       if (log_unit>0) write(log_unit,'(a)')'INFO-XML:An optional tag is found : config%output%basis_info'
       call file_load( output%basis_info, node )
       if ( trim(adjustl(output%basis_info%mode)) == '') output%basis_info%mode = "not_set"
    end if
...
!
    ...
    if( associated(node) ) then
       ...
       if (output%bond_list%mode == '') output%bond_list%mode="not_set"
...
!   type(fnode), pointer :: node
    ...
    if( trim(adjustL(value)) == "" ) value="not_set"
...
!
    ...
    value = getAttribute(file_node,"interval")
    if( value /= "" ) then
       read(unit=value,fmt=*,iostat=ierr) file%interval
       if (ierr /= 0) then
         write(*,*)'ERROR:file_restart_load(interval):value=', trim(value)
         stop
       endif
    end if
