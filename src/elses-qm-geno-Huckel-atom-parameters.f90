...
!!! c1 & c2 cause erroneous repulsive energy.
    ...
    subroutine load_default(ioAtomParameters,elem_name)
      ...
      case default
         write(*,*) 'ERROR:Thre is no default parameters for that atom:', elem_name
    ...
    subroutine write_information(iAtomParameters)
      ...
      ierr=0
      do k=1,j
        if (iAtomParameters%principal(k) < 1) ierr=1
        if (iAtomParameters%principal(k) > 10) ierr=1
        if (iAtomParameters%angular(k)   < 0) ierr=1
        if (iAtomParameters%angular(k)   > 10) ierr=1
        if (AtomParameter(i)%zeta(k)     < 0.0d0) ierr=1
        if (AtomParameter(i)%zeta(k)     > 100.0d0) ierr=1
        if (AtomParameter(i)%rescaled_zeta(k) < 0.0d0) ierr=1
        if (AtomParameter(i)%rescaled_zeta(k) > 100.0d0) ierr=1
      enddo
      if (ierr == 1) then
        write(*,*)'ERROR:Unphysical TB parameter '
        if (log_unit > 0) write(log_unit,*)'ERROR:Unphysical TB parameter '
        stop
      endif
