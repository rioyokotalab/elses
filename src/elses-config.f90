...
!                              = -3 : 'full' setting (when the tag appears as <tag> full </tag>)
  ...
  type :: ext_force_type
     ...
     real(kind(1d0))       :: field_amplitude
  end type ext_force_type

  type :: wavekernel_type
     logical               :: set
     character(len=20)     :: mode
     ...
     real(kind(1d0))       :: suppress_constant
     real(kind(1d0))       :: eigenstate_damp_constant
     ...
     ! Restrictions for setting in ELSES with wavekernel.
  ...
  type :: calc_type
     ...
     type(ext_force_type)     :: ext_force
     type(wavekernel_type)    :: wavekernel
