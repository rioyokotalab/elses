...
!
     use M_md_ext_force,        only : add_external_force !(subroutine)
     use M_md_ext_force,        only : ini_external_force !(subroutine)
!
     implicit none
...
!
     call ini_external_force  ! Initial routine for external force method
!
     if (c_system == "geno") then
       ierr=0
...
!           ---> Plot the levels and matrices (dhij, dsij)
         call output_eigenstates(trim(adjustl(config%output%basis_info%filename)), .true.)
...
!
       call add_external_force  ! on atm_force(:,:) (optional)
!
       call generate_foi
       call qm_geno_output
!      call output_levels_matrices
...
!
     if (c_system == 'C_Omata') then
       if ( matrix_generation_and_stop ) then
         call elses_set_hami 
         return
       else
         write(*,*)'ERROR:C_Omata is compatible only to the matrix generation mode'
         stop
       endif
!       
     end if
