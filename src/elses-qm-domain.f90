...
!
     if (i_verbose >= 1) then
       if (lu > 0) then
         write(lu,*)'@@@ make_booking_list'
         write(lu,*)'   noav=',noav
         write(lu,*)'r_cut_book =',r_cut_book
       endif
     endif  
!
...
!
     if (nncut /= 2) then 
       if (lu > 0) write(lu,*)'ERROR!(BKMATV)nncut=',nncut
       write(*,*)'ERROR!(BKMATV)nncut=',nncut
       stop
     endif
!
     rcut_bk=r_cut_book
     if (rcut_bk <= 1.0d0) then 
       if (lu > 0) write(lu,*) 'ERROR!(BKMATV)rcut_bk=',rcut_bk
       write(*,*)'ERROR!(BKMATV)rcut_bk=',rcut_bk
       stop
     endif
!
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
!$omp  parallel & 
!$omp& default(none) &
!$omp& shared(config, i_pbc_x, i_pbc_y, i_pbc_z) &
!$omp& shared(jsv4jsd, njsd, noav, noao, atm_position, ax, ay, az) &
!$omp& shared(ict, rcut1, lu) &
!$omp& private(dvecx,dvecy,dvecz,w) & 
!$omp& private(jsv1,jsv2,js1d,js1b,jsd) &
!$omp& private(jsv1_min, min_atom_pair_distance)
...
!
         if ((jsv1 /= jsv2) .and. (w .lt. rcut1)) then
           ...
           if (js1d .gt. noao) then
             if (lu > 0) write(lu,*)'error!(bkmatv2):js1d,noao=',js1d,noao
...
!
       if ((config%calc%calc_check%set) .and. (config%calc%calc_check%short_atom_pair_distance%set)) then
         if (min_atom_pair_distance < config%calc%calc_check%short_atom_pair_distance%abort_level  ) then
           if (lu >0) write(lu,'(a,2i10, f20.10)') 'ABORT   :Too short atom-pair distance:', &
...
&                                        jsv2, jsv1_min, min_atom_pair_distance
         ...
         if (min_atom_pair_distance < config%calc%calc_check%short_atom_pair_distance%warning_level) then
           if (lu >0) write(lu,'(a,2i10, f20.10)') 'WARNING :Too short atom-pair distance:', &
&                                        jsv2, jsv1_min, min_atom_pair_distance
...
!$omp end parallel 
!     
     if (lu > 0) write(lu,*)' ...booking is done'
!
