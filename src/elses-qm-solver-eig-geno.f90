...
!
    ...
    if (config%calc%solver%eigen_mpi%level_lowest == -1) then
      config%calc%solver%eigen_mpi%level_lowest=1
      if (i_verbose >= 1) then
        if (lu > 0) write(lu,"(a)") 'INFO(eig_mateig_wrapper):Set eigen_mpi%level_lowest=1'
      endif
    endif
    if (config%calc%solver%eigen_mpi%level_highest == -1) then
      config%calc%solver%eigen_mpi%level_highest=n_tot_base
      if (i_verbose >= 1) then
        if (lu > 0) write(lu,"(a,i10)") 'INFO(eig_mateig_wrapper):Set eigen_mpi%level_highest=',n_tot_base
      endif
    endif
...
!     if (i_verbose >= 1) then
!       if (lu > 0) write(lu,"(a)") 'INFO(eig_mateig_wrapper):ATMP is allocated and will be deallocated'
!     endif
!     deallocate(atmp)
!   endif
!   allocate(atmp(n_tot_base, n_tot_base),stat=ierr)
!   if (ierr /=0) then
!     write(*,*)'ERROR in alloc. (eig_mateig_wrapper):atmp'
!     stop
!   endif
!   if (i_verbose >= 1) then
!     if (lu > 0) write(lu,"(a)") 'INFO(eig_mateig_wrapper):ATMP is re-allocated as (n,n)'
!   endif
!
!   ierr=0
!   if (size(eigenvectors,1) /= n_tot_base) ierr=1
!   if (size(eigenvectors,2) /= n_tot_base) ierr=1
!   if (ierr /=0) then
!     write(*,*)'ERROR(eig_mateig_wrapper):size(eigenvectors,1)=',size(eigenvectors,1)
!     write(*,*)'ERROR(eig_mateig_wrapper):size(eigenvectors,2)=',size(eigenvectors,2)
!     write(*,*)'ERROR:NOTE:Now the partial eigenpair calculation is not supported'
!     stop
!   endif
!   atmp(:,:)=eigenvectors(:,:)
!   if (i_verbose >= 1) then
!     if (lu > 0) write(lu,"(a)") 'INFO(eig_mateig_wrapper):atmp=eig_vector'
!   endif
!
!
