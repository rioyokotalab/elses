...
c
      ...
      if (c_system .eq. "C_Omata") imode=3
c
      if (i_verbose >= 1) then
...
c
ccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c     Paremeter for Omata's Hamiltonian (if imode=3)
c         https://doi.org/10.1016/j.physe.2005.06.009
c
      if (imode .eq. 3) then 
c
        ierr=0
...
c       r_cut_tail=5.20d0             ! tail distance in A
c          ---> r_1
      endif  
c
ccccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
c
          if (i_partial == 0) then
            ...
            if (allocated(dfij)) dfij(:,:,:,jsd1,jsv2)=0.0d0
          endif  
c
          if (i_partial /= 0) then
