...
!
    jpe=0 ! dummy value for OMP thread ID
    fd=jpe+10
...
!   write(*,*)'filename_basic  =',trim(filename_wrk)
    call generate_output(generate, filename_wrk, 0, 0, atom_id_is_added, 0,0, fd )
!
...
!$omp& private (jpe, npe, fd) &
!$omp& private (j_split, atom_ini, atom_fin, atom_num, filename_wrk, chara_wrk) &
!$omp& shared  (n_split, noa, lenf, filename_header, generate)
    jpe=0
    npe=1
...
!       -->  atom_fin = (j_split+1) * noa / n_split
      ...
      call generate_output(generate, trim(filename_wrk), atom_ini, atom_num, atom_id_is_added, j_split, n_split, fd)
    enddo
!$omp end do
!$omp end parallel
!   
...
!
  ...
  subroutine generate_output( generate, filename, atom_ini, atom_num, atom_id_is_added, j_split, n_split, fd)
    implicit none
    type(generate_type), intent(in) :: generate
    character(len=*),    intent(in) :: filename
    ...
    integer,             intent(in) :: fd ! file unit number ( fd=10, 11, 12, ... )
...
!
    write(*,*)'generate_output:fd, j_split = ', fd, j_split
!
    ierr=0
    if (fd < 10) ierr =1
    if (fd > 99) ierr =1
    if (ierr /= 0) then
      write(*,*)'ERROR:Too large file unit number: fd  =', fd
      write(*,*)'ERROR:Too large file unit number: jpe =', 0
      stop
    endif
!
