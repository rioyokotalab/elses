...
!     write(*,*)'i, argc=',i, trim(argc)
      ...
      if (ierr /= 0) then
        if (len_trim(argc) > 1000) then
          write(*,*)'ERROR:Too long expression ? : ', trim(argc)
          stop
        endif
...
!
    ...
    if (trim(file_format) /= 'not_specified') then
      ...
      read(10,'(a)') str_wrk
      read(str_wrk,*,iostat=ierr) n_eigen_vectors, n_tot_base_wrk
      if (ierr /= 0) then
        read(str_wrk,*,iostat=ierr) n_eigen_vectors
        if (ierr /=0) then
          write(*,*)'ERROR: file reading :n_eigen_vectors'
          stop
        endif
      endif
    endif
    write(*,*) ' number of eigen vectors =', n_eigen_vectors
!
    if (n_tot_base /= n_tot_base_wrk) then
      write(*,*)'ERROR:n_tot_base, n_tot_base_wrk=', n_tot_base, n_tot_base_wrk
      stop
    endif
!
...
!
    read(10,'(a)',iostat=ierr) str_wrk
    if (ierr == 0) then
      write(*,*)'ERROR in file reading: wrong format ?'
      write(*,*)'Extra line =', trim(str_wrk)
      stop
    else
      write(*,*)'INFO:file reading is ended without error'
    endif
!
    close(10)
!
